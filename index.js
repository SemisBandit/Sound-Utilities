...
client.on('ready',  async() => {
    ...
    const statusArray = ['VALORANT, PLAYING', `Sound ', WATCHING` , '#general, WATCHING' , 'Spotify, LISTENING TO', `your Modmails, WATCHING` , 'Sound , WATCHING', `everyone, WATCHING` , 'COVID-19, WATCHING' , `100+ Users, WATCHING`]
    setInterval(() => {
...
client.on('message', async(msg) => {
	...
	var modmailchannel = msg.client.channels.cache.find(c => c.id === '891607018941612052') //place ur modmail channel id 
    ...
    if(msg.content.length >= 300 && !msg.member.hasPermission('ADMINISTRATOR')) {
        var emojiGuild = client.guilds.cache.find(guild => guild.name === 'Test Server') //type ur guild name
   ...
   return msg.reply(`${animebonk} You cannot send Bulk Messages in the Server!`)
    ...
    if(msg.content === "!verify" && msg.channel.id === "") { //type ur verify chanenl id
        msg.delete()
        const verified = msg.guild.roles.cache.find(r => r.name === "Verified")
        msg.member.roles.add(verified)
    }
    if(msg.channel.id === "" && msg.content !== "!verify") {//here also
        return msg.delete()
    }
...
return msg.reply(`${animebonk} You cannot Mass Mention People in this Server!`)
    ...
    function isValidURL(string) {
      ...
      if(isValidURL(testContent) && msg.channel.id !== '' && !msg.member.hasPermission('ADMINISTRATOR')) {
        var emojiGuild = client.guilds.cache.find(guild => guild.name === 'Test Server ') //type ur guild name
        var animebonk = emojiGuild.emojis.cache.find(emoji => emoji.name === 'animebonk')
          msg.delete();
      ...
      try {
        ...
        if(mentionedUser.id === '' && !msg.member.hasPermission('MANAGE_MESSAGES')){ //type ur id 
            msg.delete()
            ...
            .addField('Reason' , '[AutoMod] Pinging SoundDrout') //type ur name of discord
    ...
    try {
        ...
        if(mentionedUser.id === '') { //type ur bot id
            msg.reply('Hello! My name is Sound Utils. My Prefix is \`>\`.') //type ur name of server
        }
// this is extraif yo want to delete u can delete :D
...
let warnsJSON = JSON.parse(Fs.readFileSync('./warnInfo.json'));
  warnsJSON[member.id] = {
    ...
    client.on('messageUpdate' , (oldMessage, newMessage) => {
        if(newMessage.content.includes('<@>')) { //type ur bot id
            newMessage.delete()
            return newMessage.reply('Hello my Name is Sound Utilities. My prefix is `/>`/!') //change ur prefix if u want to
        }
    })
...
// ALRIGHT SO DONT FORGOT TO GIVE ME CREDIT IT TOOK ALOT OF TIME IDIOTS SO SUBSCRIBE TO NIGHTMARE AND JOIN DISCORD
// IDOTS
